...
class AccountSystem {
    constructor() {
        ...
        // URL du serveur - à adapter selon ton environnement
    ...
    getServerUrl() {
        // Si en local: utiliser localhost:3000
        if (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1') {
            return 'http://localhost:3000';
        }
        
        // Si sur GitHub Pages: essayer de se connecter au serveur local sur le même réseau
        if (window.location.hostname.includes('github.io')) {
            // Chercher l'IP locale depuis le sessionStorage (l'utilisateur peut la définir)
    ...
    promptForServerIP() {
        const ip = prompt('Adresse IP du serveur (ex: 192.168.1.100):');
        if (ip) {
